...
contract QuarkWallet is IERC1271 {
    ...
    bytes32 internal constant QUARK_OPERATION_TYPEHASH = keccak256(
        "QuarkOperation(address scriptAddress,scriptSource,bytes scriptCalldata,uint256 nonce,uint256 expiry,bool allowCallback)"
    );
        ...
        returns (bytes memory)
    {
        ...
        bytes32 structHash = keccak256(
            abi.encode(
                QUARK_OPERATION_TYPEHASH, op.scriptAddress, op.scriptSource, op.scriptCalldata, op.nonce, op.expiry, op.allowCallback
            )
        );
