...
contract QuarkWalletFactory {
    ...
    constructor() {
        ...
        quarkWalletImpl = address(new QuarkWallet(codeJar, stateManager));
    }
    ...
    function create(address signer, bytes32 salt) public returns (address payable) {
        ...
        address payable walletAddress = payable(
            address(new QuarkWalletDirectProxy{salt: keccak256(abi.encode(signer, executor, salt))}(quarkWalletImpl))
        );
        QuarkWallet(walletAddress).initialize(signer, executor);
                    ...
                    uint256(
                        keccak256(
                            abi.encodePacked(
                                bytes1(0xff),
                                address(this),
                                keccak256(abi.encode(signer, executor, salt)),
                                keccak256(
                                    abi.encodePacked(
                                        type(QuarkWalletDirectProxy).creationCode, abi.encode(quarkWalletImpl)
                                    )
                                )
                            )
                        )
